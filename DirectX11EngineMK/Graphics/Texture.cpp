...
Texture::Texture(ID3D11Device* device, const std::string& filePath, aiTextureType type)
{
	...
	{
		HRESULT hr = DirectX::CreateDDSTextureFromFile(device, StringHelper::StringToWide(filePath).c_str(), _texture.GetAddressOf(), _textureView.GetAddressOf());
		if (FAILED(hr))
		{
	...
	{
		HRESULT hr = DirectX::CreateWICTextureFromFile(device, StringHelper::StringToWide(filePath).c_str(), _texture.GetAddressOf(), _textureView.GetAddressOf());
		if (FAILED(hr))
		{
			this->Initialize1x1ColorTexture(device, Colors::UnloadedTextureColor, type);
		}
